...
First, Batect determines which tasks to run and what order to run them in, based on the prerequisites of the requested task and any prerequisites of those prerequisites and so on.
...
Then, it then creates a Docker network for the task, and runs [cache initialisation](../how-to/performance.md#cache-volumes) if required.
